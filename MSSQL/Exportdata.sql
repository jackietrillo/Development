/*
sp_export_data table
sp_export_data table,@from="from table where field='XYZ'"
sp_export_data table,@from="from table where field='XYZ'",@ommit_identity=1
*/
...
IF (SELECT OBJECT_ID('sp_export_data','P')) IS NOT NULL --means, the procedure already exists
 BEGIN
  PRINT 'Procedure already exists. So, dropping it'
  DROP PROC sp_export_data
 END
GO
...
/***********************************************************************************************************
Procedure: sp_export_data  (Build 22) 
...
PowerNote:
To generate INSERT statements for all the tables in your database, execute the following query in that database,
which will output the commands, that you need to execute for the same:
 
   SELECT 'EXEC sp_export_data ' + 
   '[' + name + ']' + 
   ',@owner = ' + 
   '[' + RTRIM(USER_NAME(uid)) + '],' + 
...
PRINT '--INSERTs generated by ''sp_export_data'' stored procedure written by Vyas'
...
PRINT 'Granting EXECUTE permission on sp_export_data to all users'
GRANT EXEC ON sp_export_data TO public
